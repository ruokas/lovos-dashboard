...
**Kas daroma Supabase.**
- Supabase SQL redaktoriuje sukurkite `materialized view` (žemiau pateiktas pavyzdys `daily_bed_metrics`) su dienos KPI: tvarkytos lovos, SLA viršijimai, vidutinis reagavimo laikas.
...
select
  date_trunc('day', coalesce(bse.created_at, oe.created_at))::date as day,
  count(distinct bse.id) filter (where bse.status is not null) as status_updates,
  count(distinct oe.id) filter (where oe.occupancy_state is not null) as occupancy_updates,
  avg(extract(epoch from (oe.created_at - bse.created_at)) / 60.0)
    filter (
      where oe.created_at is not null
        and bse.created_at is not null
        and oe.created_at >= bse.created_at
    ) as avg_minutes_between_status_and_occupancy,
  count(*) filter (
    where bse.priority >= 2
      and (oe.created_at is null or oe.created_at - bse.created_at > interval '30 minutes')
  ) as sla_breaches
from public.beds b
left join public.bed_status_events bse on bse.bed_id = b.id
left join public.occupancy_events oe on oe.bed_id = b.id
group by day
order by day desc;
