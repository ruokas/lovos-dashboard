...
export class SettingsUI {
  constructor(settingsManager, onSettingsChange, options = {}) {
    this.settingsManager = settingsManager;
    this.onSettingsChange = onSettingsChange;
    this.isOpen = false;
    this.modal = null;
    this.onClearLocalData = options.onClearLocalData ?? null;
      ...
      <div class="bg-white dark:bg-slate-800 rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        <div class="p-6">
          ...
          <form id="settingsForm" class="space-y-6">
            ...
            <!-- Actions -->
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 pt-6 border-t border-slate-200 dark:border-slate-600">
              <div class="flex flex-col sm:flex-row gap-3">
                <button
                  type="button"
                  id="clearLocalData"
                  class="px-4 py-2 text-sm font-medium text-red-600 dark:text-red-300 bg-red-50 dark:bg-red-900/30 hover:bg-red-100 dark:hover:bg-red-900/50 rounded-md transition-colors"
                >
                  Išvalyti vietinę talpyklą
                </button>
                <button
                  type="button"
                  id="resetSettings"
  ...
  attachEventListeners() {
    ...
    const clearLocalDataBtn = this.modal.querySelector('#clearLocalData');
    if (clearLocalDataBtn) {
      clearLocalDataBtn.addEventListener('click', () => {
        if (confirm('Išvalyti vietinius duomenis (įskaitant užduotis ir formas)?')) {
          if (typeof this.onClearLocalData === 'function') {
