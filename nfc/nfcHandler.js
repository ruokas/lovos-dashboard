...
export class NfcHandler {
  ...
  async processCurrentTag() {
    ...
    await this.logger?.logInteraction?.('nfc_tag_detected', { payload: { tagCode: tag } });
    ...
    try {
      ...
      if (!record || !record.bed?.label) {
        ...
        await this.logger?.logInteraction?.('nfc_tag_unmatched', { payload: { tagCode: tag } });
        return;
      }
      ...
      await this.logger?.logInteraction?.('nfc_tag_matched', {
        ...
        payload: { tagCode: tag, bedLabel },
      });
    } catch (error) {
      console.error('NFC žymos apdorojimo klaida:', error);
      this.form?.show(null, { trigger: 'nfc' });
      this.form?.showFeedback(t(texts.nfc.lookupFailed), 'error');
      await this.logger?.logInteraction?.('nfc_tag_error', { payload: { tagCode: tag, error: error.message } });
    } finally {
      this.#clearTagParam();
    }
  }
